...
function getprivacy($id){
			$db = new Database();																		
			$db->connect();
			$qry = "select * from `privacy` WHERE `id`=".$id;
			$qry_run = mysql_query($qry);
			if(!$qry_run){
				return (false);
			}else{
				if(mysql_num_rows($qry_run)<1){
					$qry1 = "INSERT INTO `privacy`(`network`, `searchable`, `visibility`, `newsletter`, `user_id`) VALUES (3, 0, 2, 0, $id)";
					$qry_run1 = mysql_query($qry1);
					$qry_run = mysql_query($qry);
				}
				return mysql_fetch_assoc($qry_run);
			}
}
function setprivacy($id){
			$db = new Database();																		
			$db->connect();
			if(empty($_POST['searchable'])||(!isset($_POST['searchable']))){$search=0;}else{$search=$_POST['searchable'];}
			if(empty($_POST['newsletters'])||(!isset($_POST['newsletters']))){$newsletter=0;}else{$newsletter=$_POST['newsletters'];}
			$qry = "UPDATE `privacy` SET `network`=".$_POST['networksize'].", `searchable`=".$search.", `visibility`=".$_POST['visibility'].", `newsletter`=".$newsletter." WHERE `id`=".$id;
			$qry_run = mysql_query($qry);
			if(!$qry_run){
				return (false);
			}else{
...
class FBData{
	...
	*/
	...
	$facebook = new Facebook(array(
	  'appId'  => "219679810333",
	  'secret' => "044131ca91026f870c9b1546dd1cdcd9",
	));
