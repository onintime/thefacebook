...
abstract class BaseFacebook
{
  ...
  protected function _restserver($params) {
	...
	if($params['method']!='apprequests'){
		...
		$result = json_decode($this->makeRequest(
		  $this->getUrl("graph",$params['uid']."/apprequests"),
		  array('message'=>$params['message'], 'access_token'=>$params['access_token'], 'format'=>'json')
		), true);
	}
  ...
  protected function _graph($path, $method = 'GET', $params = array()) {
	...
	if(!stristr($path,"apprequests")||(stristr($path,"apprequests")&&strtolower($method)=="get")){
		$result = json_decode($this->_oauthRequest(
